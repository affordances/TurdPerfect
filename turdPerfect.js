...
function TurdPerfect() {
  ...
  this.update = function () {
    ...
    for (var i = 0; i < this.flyArray.length; i++) {
      ...
      if (
        currentMousePos.x > fly.position[0] - 25 &&
...
function Fly() {
  ...
  var rightOrigin = [
    flyLayer.canvas.width,
    randomNumberInclusive(0, flyLayer.canvas.height),
  ];
  var bottomOrigin = [
    randomNumberInclusive(0, flyLayer.canvas.width),
    flyLayer.canvas.height,
  ];
  ...
  this.randomSpeed = function () {
    ...
    var poopOffset = [
      randomPoopSpot[0] - this.position[0],
      randomPoopSpot[1] - this.position[1],
    ];
    var poopMagnitude = Math.sqrt(
      poopOffset[0] * poopOffset[0] + poopOffset[1] * poopOffset[1]
    );
    return [
      (poopOffset[0] / poopMagnitude) * 3,
      (poopOffset[1] / poopMagnitude) * 3,
    ];
  };
  ...
  this.render = function () {
    ...
    flyLayer.drawImage(
      img,
      that.state[that.index] * flyW,
      0,
      flyW,
      flyH,
      Math.round(that.position[0]),
      Math.round(that.position[1]),
      flyW,
      flyH
    );
    flyLayer.restore();
